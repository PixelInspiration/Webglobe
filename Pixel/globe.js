...
DAT.Globe = function (container, opts) {
  ...
  function addCity(lat, lng, size, city, color, img, slideshowURL) {
	 ...
	 */
	function setActiveRegion(country) {
		country = country || wimSettings.country;
		target.x = wimSettings[country].x;
    target.y = wimSettings[country].y;
    allRegions.removeClass('active');
    
    var country_name = wimSettings[country].name === 'US' ? 'USA' : wimSettings[country].name;
    if (country_name === 'MENAT') {
    	country_name = 'Middle East';
    }
    activeReg.html(country_name);
    $('.'+wimSettings[country].name.toLowerCase()).addClass('active');
	}
  ...
  //Event listeners for regions
    ...
    track.trackInteraction(country+" Filter (all regions - new)")
	});    
	
	  
  //THEMES FILTER
	var internetT = $('.t-industrial-internet'),
			energyT = $('.t-energy'),
			healthT	= $('.t-healthcare'),
			skillsT = $('.t-skills-work'),
			transportationT = $('.t-transportation'),
			manufacturingT = $('.t-manufacturing'),
			infrastructureT = $('.t-infrastructure'),
			gestoreT = $('.t-gestore');

	internetT.click(function() {
		track.trackInteraction("Internet Filter (new)");
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].theme === 'industrial internet') {

					dotMesh[index].themeState = 'off';

					if (dotMesh[index].formatState !== 'off') {
	...
	energyT.click(function() {
		track.trackInteraction("Energy Filter (new)")
		$(this).toggleClass('off');
	...
	healthT.click(function() {
		track.trackInteraction("Health Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].theme === 'healthcare') {
	...
	skillsT.click(function() {
		track.trackInteraction("Skills Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].theme === 'skills & work') {
	...
	transportationT.click(function() {
		track.trackInteraction("Transportation Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].theme === 'transportation') {
	...
	manufacturingT.click(function() {
		track.trackInteraction("Manufacturing Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].theme === 'manufacturing') {
	...
	infrastructureT.click(function() {
		track.trackInteraction("Infrastructure Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].theme === 'infrastructure') {
	...
	gestoreT.click(function() {
		track.trackInteraction("GE Store Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].theme === 'gestore') {
	...
	//FORMAT FILTER//
	var articleF = $('.f-article'),
		slideshowF = $('.f-slideshow'),
		videoF = $('.f-video'),
		infographicF = $('.f-infographic'),
		socialF = $('.f-social');
			
	//Article		
	articleF.click(function() {
		track.trackInteraction("Article Filter (new)")
		$(this).toggleClass('off');
	...
	//Slideshow	
	slideshowF.click(function() {
		track.trackInteraction("Slideshow Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].format === 'slideshow') {
	...
	videoF.click(function() {
		track.trackInteraction("Video Filter (new)")
		$(this).toggleClass('off');
	...
	infographicF.click(function() {
		track.trackInteraction("Inforgraphic Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].format === 'infographic') {
	...
	socialF.click(function() {
		track.trackInteraction("Social Filter (new)")
		$(this).toggleClass('off');
		...
		if ($(this).hasClass('off')) {
			...
			$.each(dotMesh, function(index, value) {
				...
				if (dotMesh[index].format === 'social') {
 ...
 //ON/OFF SWITCH//
  var allThemesItems = $('.t-industrial-internet, .t-energy, .t-healthcare, .t-skills-work, .t-transportation, .t-manufacturing, .t-infrastructure, .t-gestore'),
    allThemes = $('.all-filters');
	
  allThemes.click(function() {
    track.trackInteraction("All Filters (on/off)");
    $(this).toggleClass('off');
      
    if ($(this).hasClass('off')) {
      allThemesItems.addClass('off');
      ...
      allThemesItems.removeClass('off');
  ...
  //Check if all items are turned off to add class to on/off switch
  allThemesItems.click(function () {
    if ( allThemesItems.length === $('.themes .off').length ){
      allThemes.addClass('off');
    } else {
      allThemes.removeClass('off');
    }
  });
  ...
  function onMouseDown(event) {
  	track.trackInteraction('Globe Interaction (new)');
  ...
  function onMouseUp(event) {
    ...
    if (activeCity != -1) {
      ...
      track.trackInteraction('Content Card (new)')
      ...
      popup.attr('data-content-name', aTitle.html());
		 ...
		 arrowNav = (function() {
            ...
            var card2 = testArr[currentDot][5]
                  ...
                  track.trackInteraction(card2+" - Video Opened")
									videoContainer.show();
								});
      ...
      if(cities[activeCity].format === 'video') {
        videoContainer.html("<iframe width='531' height='387' src=" + "'" + cities[activeCity].link + "'" + "frameborder='0' allowfullscreen></iframe>");
       	aLink.hide();
				videoLink.show();
				videoLink.on('click', function() {
          track.trackInteraction(card+" - Video Opened")
					videoContainer.show();
				});
	...
	//Content card arrow navigation	
	 $('.nav-arrow-next').on('click', function() {
	 		track.trackInteraction("Content Card next arrow clicked (new)");
	 	  arrowNav.next();
	 });

	  $('.nav-arrow-prev').on('click', function() {
	  	track.trackInteraction("Content Card prev arrow clicked (new)");
	 	  arrowNav.prev();
	 });
	  
	 //image slideshow navigation
	 $('.next').on('click', function() {
	 	track.trackInteraction("Inner slideshow clicked (new)");
	 	  slideshowStart.next();
	 });
  ...
  function onWindowResize(event) {
		...
		window.innerWidth = $('#qzad').width(); //needed to center globe in our container: for dev only
...
initCard = function() {
	...
	setTimeout(function() {
	  ...
	  var k = Math.floor(Math.random() * tempArr.length);
		...
		var themes = tempArr[k][8];
		
			switch (themes){
				case 'industrial internet': 
				aLink.css('background-color', '#f7c438');
				tagTheme.css('color', '#f7c438');
				break;
				case 'energy': 
				aLink.css('background-color', '#d2fd3a');
				tagTheme.css('color', '#d2fd3a');
				break;
				case 'healthcare': 
				aLink.css('background-color', '#d77fe8');	
				tagTheme.css('color', '#d77fe8');	
				break;
				case 'skills & work': 
				aLink.css('background-color', '#4edee2');
				tagTheme.css('color', '#4edee2');
				break;
				case 'transportation': 
				aLink.css('background-color', '#ff8046');
				tagTheme.css('color', '#ff8046');
				break;
				case 'manufacturing': 
				aLink.css('background-color', '#d3217a');
				tagTheme.css('color', '#d3217a');
				break;
				case 'infrastructure': 
				aLink.css('background-color', '#35ba8a');
				tagTheme.css('color', '#35ba8a');
				break;
			}
